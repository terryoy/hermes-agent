...
def test_normalize_path_collapses_symlink_alias(tmp_path: Path):
    """Two gateway sessions must share one LSP workspace through a symlink."""
    real_repo = tmp_path / "real-repo"
    real_repo.mkdir()
    alias = tmp_path / "alias-repo"
    alias.symlink_to(real_repo, target_is_directory=True)

    assert normalize_path(str(alias / "frontend")) == normalize_path(
        str(real_repo / "frontend")
    )
